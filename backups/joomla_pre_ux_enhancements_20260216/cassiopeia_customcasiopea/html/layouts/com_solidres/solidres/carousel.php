            ...
            <div class="<?php echo SR_UI_CAROUSEL_ITEM ?> <?php echo $i == 0 ? 'active' : '' ?>">
                <?php if (!empty($linkItem)): ?>
                    <a class="sr-photo-<?php echo $objectId ?> <?php echo $linkClass ?? '' ?>"
                        href="<?php echo $linkUrl ?? $image; ?>" data-fitview="true" <?php echo $linkAttr ?? '' ?>>
                    <?php endif ?>
                    <img src="<?php echo $thumb; ?>" alt="<?php echo $objectName ?? basename($thumb) ?>"
    ...
    <?php if (isset($showIndicators) && $showIndicators): ?>
        ...
        <div class="carousel-indicators">
            <?php for ($i = 0, $n = count($items); $i < $n; $i++): ?>
                <button type="button" data-bs-target="#<?php echo $id ?>" data-bs-slide-to="<?php echo $i ?>" class="active"
                    aria-current="true" aria-label="<?php echo $i ?>"></button>
            <?php endfor; ?>
        </div>

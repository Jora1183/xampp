...
class SolidresModelCustomer extends SRModelAdmin
{
     ...
     * @since    1.6
     */
    public function getTable($type = 'Customer', $prefix = 'SolidresTable', $config = [])
    {
    ...
    protected function loadFormData()
    {
        ...
        $data = JFactory::getApplication()->getUserState('com_solidres.edit.customer.data', []);
        ...
        JFactory::getApplication()->triggerEvent('onContentPrepareData', ['com_solidres.customer', $data]);
    ...
    public function getItem($pk = null)
    {
        ...
        if ($item->id) {
            ...
            // Trigger the data preparation event.
            JFactory::getApplication()->triggerEvent('onContentPrepareData', ['com_solidres.customer', $item]);
        }
    ...
    public function save($data)
    {
        ...
        // Trigger the onCustomerBeforeSave event, create new Joomla user first
        $result = $app->triggerEvent($this->event_before_save, [$data, $table, $isNew, &$response]);

        if (SRPlugin::isEnabled('customfield') && $isAdmin) {
            return true; // Return here as the customer saving process is done in the $this->event_before_save > saveUserProfile()
        }
        ...
        $result = $app->triggerEvent($this->event_after_save, [$data, $table, $result, $isNew]);
        if (\in_array(false, $result, true)) {
            $this->setError($table->getError());

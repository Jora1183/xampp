...
const venoboxLoader = (selector, options) => {
    const initOptions = {
        type: undefined,
        numeration: true,
        infinigall: true,
    ...
    const { type } = initOptions;
    const elements = document.querySelectorAll(selector);

    elements.forEach(element => {
        if (element.classList.contains('venobox-loaded')) {
            return;
        }

        element.classList.add('venobox-loaded');
        const validTypes = ['iframe', 'ajax', 'inline'];

        if (validTypes.includes(type)) {
            element.setAttribute('data-vbtype', type);
        } else if (element.getAttribute('data-venobox') === 'gallery') {
            const gall = 'sr-gallery-' + Math.floor(100000000 + Math.random() * 900000000);
            element.querySelectorAll('a[href]').forEach(a => {
                if (a.href.match(/\.(jpe?g|png|gif|webp|sbg)$/i)) {
                    a.setAttribute('data-gall', gall);
                }
            });
        }
    });

    if (selector === '[data-venobox="gallery"]') {
        selector += ' a[href]';
    }

    new VenoBox({
        selector,
        jQuerySelectors: initOptions.container instanceof HTMLElement ? initOptions.container.querySelectorAll(selector) : null,
        numeration: initOptions.numeration,
        infinigall: initOptions.infinigall,
        share: initOptions.share,
        maxWidth: initOptions.maxWidth,
        spinner: 'rotating-plane',
    });
};

const initVenobox = function (container) {
    if (container) {
        ...
        if (!(container instanceof HTMLElement)) {
            console.debug(`Container isn't a valid HTMLElement`, container);
            return;
        }
    }

    const init = () => {
        venoboxLoader('[data-venobox="ajax"]', { type: 'ajax', container });
        venoboxLoader('[data-venobox="inline"]', { type: 'inline', container });
        venoboxLoader('[data-venobox="iframe"]', { type: 'iframe', container });
        venoboxLoader('[data-venobox="gallery"]', { container });
    };

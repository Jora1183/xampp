...
class SRRoomType
{
	...
	public function getPriceDaily($tariffWithDetails, $roomTypeId, $checkin, $checkout, $imposedTaxTypes, SRCurrency $solidresCurrency, $coupon = null, $adultNumber = 0, $childAges = [], $stayLength = 0, $discounts = [], $isDiscountPreTax = false, $config = [])
	{
		...
		$totalSingleSupplementFormatted                   = null;
		$totalTaxAmountDiscounted                         = 0;
		$totalTaxAmountDiscountedFormatted                = null;
		...
		if ($totalBookingCost > 0 || (isset($config['allow_free']) && $config['allow_free']))
		{
			...
			$totalTaxAmountDiscountedFormatted = clone $solidresCurrency;
			$totalTaxAmountDiscounted = $totalBookingCostTaxInclDiscounted - $totalBookingCostTaxExclDiscounted;
			$totalTaxAmountDiscountedFormatted->setValue($totalTaxAmountDiscounted);
		...
		$response = [
			...
			'total_discount'                            => $totalDiscount,
			'total_discount_formatted'                  => $totalDiscountFormatted,
			'total_tax'                                 => $totalTaxAmountDiscounted,
			'total_tax_formatted'                       => $totalTaxAmountDiscountedFormatted,
			'applied_discounts'                         => $appliedDiscounts,
			'tariff_break_down'                         => $tariffBreakDown,
			...
			'is_discount_pre_tax'                       => $isDiscountPreTax,
		];
	...
	private function calculateCostPerDay($tariff, $bookWeekDay, $coupon = null, $adultNumber = 0, $childAges = [], $imposedTaxTypes = [], $config = [])
	{
		...
		{
			if ($tariff->mode == RATE_PLAN_MODE_7DAY_WEEK && is_array($tariff->details)) // 7-day week
			{
		...
		elseif ($tariff->type == PER_PERSON_PER_NIGHT)
		{
			...
			switch ($config['child_room_cost_calc'])
			{
				case 0: // Quantity
					...
					{
						$guestType = 'child' . ($i + 1);
						if ($tariff->mode == RATE_PLAN_MODE_7DAY_WEEK && is_array($tariff->details[$guestType]))
						{
				...
				case 1: // Age range
					for ($i = 0; $i < count($childAges); $i++)
					{
						foreach ($tariff->details as $guestType => $guesTypeTariff)
						{
							...
							if ($tariff->mode == RATE_PLAN_MODE_7DAY_WEEK && is_array($tariff->details))
							{
								for ($t = 0, $count = count($tariff->details[$guestType]); $t < $count; $t++)
								{
									if
					...
					foreach ($childAgeGroups as $childAgeGroup)
					{
						foreach ($childAgeGroup as $childIndex => $childAge)
						{
							foreach ($newTariffDetails as $ageRange => $newDetails)
							{
								...
								if ($tariff->mode == RATE_PLAN_MODE_7DAY_WEEK && is_array($newDetails[$guestType]))
								{
	...
	private function isValid($tariffWithDetails, $checkin, $checkout, $stayLength, $checkinDayInfo)
	{
		...
		else
		{
            ...
            $limitCheckin = [];
            if (isset($tariffWithDetails->limit_checkin) && is_string($tariffWithDetails->limit_checkin))
            {
                $limitCheckin = json_decode($tariffWithDetails->limit_checkin, true);
            }
			elseif (is_array($tariffWithDetails->limit_checkin))
			{
				$limitCheckin = $tariffWithDetails->limit_checkin;
			}

			if (
				strtotime($tariffWithDetails->valid_from) <= strtotime($checkin) &&
				strtotime($tariffWithDetails->valid_to) >= strtotime($checkout) &&
				(in_array($checkinDayInfo['wday'], $limitCheckin)) &&
				$isValidDayRange
			)
			{
				$isValid = true;
